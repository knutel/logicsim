...
- **`logicsim/graph_data.py`**: Complete graph data structures with nodes, connectors, nets, and selection state
...
- **Net lines**: Lines connect between specific connectors on nodes
...
- **NetData struct**: Defines net properties (id, start/end coordinates)
- **Dynamic rendering**: Uses `for` loops to render nodes and nets from data arrays
- **Unified rendering**: All node types rendered as rectangles with color differentiation

### Key Implementation Details

#### Slint Path Elements for Lines
- Net lines use Slint's `Path` element with `MoveTo` and `LineTo` commands
...
- **Slint format**: `to_slint_format()` includes nodes, nets, and selected_nodes arrays
